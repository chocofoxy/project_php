  ...
  <div class="bg-white w-full mt-4 mx-4  shadow-md">
    <nav class="flex flex-col content-center justify-center sm:flex-row">
      <a href="./dashboard.php" class="text-gray-600 uppercase py-4 px-6 block hover:text-blue-500 focus:outline-none">
        ...
        les etudiants
      </a><a href="./dashboard_enseignant.php" class="text-gray-600 py-4 px-6 block uppercase hover:text-blue-500 focus:outline-none text-blue-500 border-b-2 font-medium border-blue-500">
        les enseignants
      </a>
    </nav>
  </div>
  <div class="bg-white w-full mt-4 mx-4 px-8 pt-2 shadow-md">
    <form method=POST action='./add_enseignant.php' >
    <div class="flex flex-wrap -mx-3 mb-2">
      <div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
        <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="grid-city">
          matricule
        </label>
        <input name="matricule" required class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-city" type="number" min="0" placeholder="2">
      </div>
      <div class="w-full md:w-1/3 px-3 mb-6 md:mb-0">
  ...
  <div class="w-full h-full m-4">
    <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
      <table class="table-fixed w-full">
        <thead>
          <tr>
            <th class="w-1/6 px-4 py-2">Matricule</th>
        ...
        <tbody>
          <?php
            $res = $conn->query("SELECT * FROM enseignant ")->fetchAll();
             if ( !$res || count($res) == 0) {
               echo "<tr><td class='border px-4 py-4 text-center italic' colspan=6 > Il n'y a pas des enseignants </td></tr>" ;
             } else {
               $table = '' ;
               foreach ($res as $enseignant) {
                 $table .= '<tr><td class="text-center border px-4 py-2">'.$enseignant['matricule'].'</td>' .
                           '<td class="text-center border px-4 capitalize py-2">'.$enseignant['nom_Ens'].'</td>' .
                           '<td class="text-center border px-4 capitalize py-2">'.$enseignant['prenom_Ens'].'</td>' .
                           '<td class="text-center border px-4 py-2 flex justify-center">
                           <a href="./delete_enseignant.php?id='.$enseignant['matricule'].'">
